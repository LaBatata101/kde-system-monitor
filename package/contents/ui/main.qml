...
PlasmoidItem {
    ...
    function compactToolTipSummary(sectionKey) {
        switch (sectionKey) {
        case "cpu":
            return "Usage " + formatPercent(cpuTotal) + " (user " + formatPercent(cpuUser) + ", system " + formatPercent(cpuSystem) + ")" + " @ " + cpuClockText();
        case "ram":
            return ramTotal > 0 ? "Used " + formatMemoryMib(ramUsed) + " / " + formatMemoryMib(ramTotal) + " (" + formatPercent(ramUsed / ramTotal * 100) + ")" : "Memory usage unavailable";
        case "network":
            return "Up " + netUploadSpeed + ", down " + netDownloadSpeed;
        case "storage":
            return storageDevices.length > 0 ? storageDevices[0].used + " / " + storageDevices[0].size + " (" + storageDevices[0].percent + "%)" : "Storage usage unavailable";
        case "temps":
    ...
    function compactTemperaturesSummary() {
        ...
        for (var i = 1; i < temperatures.length; i++) {
            if (temperatures[i].value > hottest.value)
    ...
    P5Support.DataSource {
        ...
        onNewData: function (src, data) {
            ...
            var out = data["stdout"] || "";
            if (src === _sysfsGpuCommand || src.indexOf("gpu_busy_percent") !== -1)
                parseSysfsGpu(out);
            else if (src === _nvidiaGpuCommand || src.indexOf("--query-gpu") !== -1)
                parseNvidiaGpu(out);
            else if (src === _nvidiaGpuProcessesCommand || src.indexOf("--query-compute-apps") !== -1)
                parseNvidiaGpuProcesses(out);
            else if (src === _drmGpuProcessesCommand || src.indexOf("/proc/[0-9]") !== -1)
                parseDrmGpuProcesses(out);
        ...
        try {
            ...
            function usageForNode(node) {
                ...
                for (var i = 0; i < storageDevices.length; i++) {
                    if (storageDevices[i].device === path)
